      ...
      {viewMode === 'grouped' && (
        <div className="relative group/scroll mb-8">
          ...
          {canScrollLeft && (
            <button
              onClick={() => scroll('left')}
              className="absolute left-0 top-1/2 -translate-y-1/2 z-20 bg-white/90 backdrop-blur-sm border border-gray-200 rounded-full p-2 shadow-xl hover:bg-white transition-all -ml-4 ring-1 ring-black/5"
          ...
          {canScrollRight && (
            <button
              ...
              className="absolute right-0 top-1/2 -translate-y-1/2 z-20 bg-white/90 backdrop-blur-sm border border-gray-200 rounded-full p-2 shadow-xl hover:bg-white transition-all -mr-4 ring-1 ring-black/5"
          ...
          <div
            ...
            className="flex items-start gap-4 overflow-x-auto pb-2 no-scrollbar scroll-smooth -mx-4 px-4 sm:mx-0 sm:px-0"
          >
            {/* Filter Trigger Card */}
            <button
              onClick={() => setIsFilterSheetOpen(true)}
              className={`flex-shrink-0 flex flex-col items-center justify-center w-32 h-[104px] rounded-xl border-2 border-dashed transition-all ${activeFilterCount > 0
      ...
      <div className="mt-8 text-sm text-gray-400 flex items-center justify-center border-t border-gray-100 pt-8">
        {viewMode === 'grouped' ? (
          <p>
            ...
            <span className="font-semibold text-gray-600">{filteredGroupedReceipts.reduce((sum, g) => sum + g.receipts.length, 0)}</span> total entries
          </p>
        ) : (
          <p>
            Showing <span className="font-semibold text-gray-600">{receipts.length}</span> stock card entries
            {(() => {
